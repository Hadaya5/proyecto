...
def profile(userid=None):
    ...
    if(userid):
        ...
        if(not user):
            ...
            return render_template('profile.html',text=text[lang],user=user,editable=False,posts=db.getFriendsPosts([db.convertId(userid) ]),users=db.getUsersDict())
    else:
        user = getUser(uid)
        return render_template('profile.html',text=text[lang],user=user,editable=True,posts=db.getFriendsPosts([uid]),users=db.getUsersDict())
